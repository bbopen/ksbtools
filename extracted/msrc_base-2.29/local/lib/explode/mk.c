...
/*@Explode temp@*/
/* Look for a file, but don't make it (allows a race sometimes)		(ksb)
 */
static int
_chk_temp(char *pcTry, void *pvNull)	/*ARGSUSED*/
{
	auto struct stat stExist;

	if (-1 == lstat(pcTry, & stExist) && ENOENT == errno) {
		return 1;
	}
	errno = EEXIST;
	return 0;
...
mktemp(char *pcTempl)
{
	return _mktemp(pcTempl, 0, _chk_temp, (void *)0);
}
 ...
 */
int
mkstemp(char *pcTempl)
{
	auto int iFd = -1;

	if ((char *)0 != _mktemp(pcTempl, 0, _chk_stemp, (void *)&iFd))
		return iFd;
	return -1;
}
...
/*@Explode stemps@*/
int
mkstemps(char *pcTempl, int iSuffix)
{
	auto int iFd = -1;

	if ((char *)0 != _mktemp(pcTempl, iSuffix+1, _chk_stemp, (void *)&iFd))
		return iFd;
	return -1;
}
...
/*@Explode dtemps@*/
char *
mkdtemps(char *pcTempl, int iSuffix)
{
	return _mktemp(pcTempl, iSuffix+1, _chk_dtemp, (void *)0);
}

/*@Explode utemps@*/
/* make a unix domain socket in a mktemp-style random filename		(ksb)
 */
#include <sys/socket.h>
#include <sys/un.h>
#include <unistd.h>

/* function for above to look for a socket by binding to it		(ksb)
 */
static int
_chk_utemp(char *pcTry, void *pvFd)
{
	register int *piFd = (int *)pvFd;
	auto struct sockaddr_un run;

	if (-1 == (*piFd = socket(AF_UNIX, SOCK_STREAM, 0))) {
		return 1;
	}
	run.sun_family = AF_UNIX;
	(void)strncpy(run.sun_path, pcTry, sizeof(run.sun_path));
	if (-1 == bind(*piFd, (struct sockaddr *)&run, strlen(pcTry)+sizeof(short int))) {
		close(*piFd);
		*piFd = -1;
		errno = ENOENT;
		return 0;
...
/* the ksb-only mkutemp interface					(ksb)
 */
int
mkutemp(char *pcTempl)
{
	auto int iFd = -1;

	if ((char *)0 != _mktemp(pcTempl, 0, _chk_utemp, (void *)&iFd))
		return iFd;
	return -1;
}

/* mkutemp with a suffix interface					(ksb)
 */
int
mkutemps(char *pcTempl, int iSuffix)
{
	auto int iFd = -1;

	if ((char *)0 != _mktemp(pcTempl, iSuffix+1, _chk_utemp, (void *)&iFd))
		return iFd;
	return -1;
}
...
char **argv;
{
	static char *apcTest[] = {
		...
		"____",
		"/tmp/____",
		"/tmp/no-such-Diretory/XXXXXX",
		(char *)0
	};
	register char **ppc, *pcDup;
	register int iFd;

	for (ppc = (argc > 1) ? argv+1 : apcTest; (char *)0 != *ppc; ++ppc) {
		pcDup = strdup(*ppc);
		printf("mkdtemp(%s)", pcDup);
		if ((char *)0 == mkdtemp(pcDup)) {
			printf(" failed with %d (partial \"%s\")", errno, pcDup);
		} else {
			printf(" returns %s", pcDup);
			if (0 == rmdir(pcDup))
				printf(" (removed)");
		}
		printf("\n");
	...
	/* Just to show I did the silly suffix hack
	 */
	pcDup = strdup("/tmp/XXXXXX.XYZ");
	printf("mkstemp(%s, 5)", pcDup);
	iFd = mkstemps(pcDup, 5);
	if (-1 == iFd) {
		printf(" failed with %d (partial \"%s\")", errno, pcDup);
	} else {
		printf(" returns %s", pcDup);
	...
	/* show how to get a wrapper socket, cleverly
	 */
	pcDup = strdup("/tmp/wrapXXXXXX");
	printf("mkutemp(%s)", pcDup);
	iFd = mkutemp(pcDup);
	if (-1 == iFd) {
		printf(" failed with %d (partial \"%s\")", errno, pcDup);
	} else {
		printf(" returns %s (fd=%d)", pcDup, iFd);
		close(iFd);
		if (0 == unlink(pcDup)) {
			printf(" (removed)");
		}
