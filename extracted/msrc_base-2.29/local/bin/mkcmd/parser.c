/* $Id: parser.c,v 8.41 2009/10/14 16:18:03 ksb Exp $
...
KEY **ppKVInstall;
{
	...
	if (STRING == TokCur) {
		pcTmp = UseChars(& sLen);
		if (1 != strlen(pcTmp)) {
			fprintf(stderr, "%s: %s(%d) key \"%s\" alias ('%s') must be a single character (length %u)\n", progname, pchScanFile, iLine, pcName, pcTmp, (unsigned)strlen(pcTmp));
		} else {
			cKey = pcTmp[0];
		}
		TokCur = GetTok();
...
/* Pass me in a mktemp template, I'll instance it and add it to		(ksb)
 * the list of temp files to cleanup on exit
 */
FILE *
MakeDivert(char *pcTemp, int cType)
{
	register int iFd;
	register FILE *fpRet;
	...
	if (-1 == (iFd = mkstemp(pcTemp))) {
		fprintf(stderr, "%s: mkdtemp: %d: %s\n", progname, iFd, strerror(errno));
		exit(MER_BROKEN);
	}
	if ((FILE *)0 == (fpRet = fdopen(iFd, "w+"))) {
		fprintf(stderr, acFailOpen, progname, pcTemp, strerror(errno));
		exit(MER_BROKEN);
	}
	(void)ListAdd(& LIDelete, pcTemp);
	switch (cType) {
	case 'c':
		(void)ListAdd(& LIMetaC, pcTemp);
		break;
	case 'h':
		(void)ListAdd(& LIMetaH, pcTemp);
		break;
	case 'i':
		(void)ListAdd(& LIMetaI, pcTemp);
		break;
	case 'm':
		(void)ListAdd(& LIMetaM, pcTemp);
		break;
	case EOF:	/* explicity none */
		break;
...
char *pcVersion;
{
	register int i;
	static char
		acCTmp[] = "/tmp/mkscXXXXXX",
		acHTmp[] = "/tmp/mkshXXXXXX",
		acTTmp[] = "/tmp/mksiXXXXXX";
	...
	fpDivert = MakeDivert(acCTmp, EOF);
	fpHeader = MakeDivert(acHTmp, EOF);
	fpTop = MakeDivert(acTTmp, EOF);
}
...
/* a kinda R.E. match, spaces match 0 or more isspace			(ksb)
 */
static char *
KindaLike(pcHave, pcState)
char *pcHave, *pcState;
{
	register int c;

	for (/* param */; '\000' != (c = *pcHave); ++pcHave) {
		switch (*pcState) {
		case '\t':
		case ' ':
			if (isspace(*pcHave)) {
		...
		default:
			...
			if (c == (isupper(*pcState) ? tolower(*pcState) : *pcState)) {
				break;
			}
			return (char *)0;
		}
	...
	while (isspace(*pcState)) {
		++pcState;
	}
	return '\000' == *pcState ? pcHave : (char *)0;
}

/* RPCGen (on all platforms I can find) emits the same broken code	(ksb) 
 *
 * In -l's client code there is a global static declaration for TIMOUT:
 *	"static struct timeval TIMEOUT = { 25, 0 };"
 * when we integrate 2 .x clients into a single mkcmd main.c we find that
 * the second global conficts with the first.  Sigh.
 * Moreover the declaration is below the code that uses it in some cases
 * so we must move (replicate) it at the fpTop section...
 */
static char acHackMe[] =
	"static struct timeval TIMEOUT = { 25, 0 };";
static void
RpcTimeout(fp)
FILE *fp;
{
	static char acHackOnce[] = "U_RPC_TIMEONCE";

	Emit(fp, "#if !defined(%a)\n", nilOR, acHackOnce, 0);
	Emit(fp, "#define %a 1\n", nilOR, acHackOnce, 0);
	Emit(fp, "%a\n", nilOR, acHackMe, 0);
	Emit(fp, "#endif /* %a */\n", nilOR, acHackOnce, 0);
}

/* remove the #include's from an RPC slug and move them to		(ksb)
 * our include section
 */
static void
RpcRemember(pcBuf)
char *pcBuf;
{
	 ...
	 */
	for (pcEnd = pcBuf; '\000' != *pcEnd; ++pcEnd) {
		if ('/' != pcEnd[0] || '*' != pcEnd[1])
			continue;
		*pcEnd = '\000';
		break;
	 ...
	 */
	if ('"' != pcBuf[0] && '\000' != pcBuf[0]) {
		ListAdd(& LISysIncl, strdup(pcBuf));
	}
}
...
FILE *fpOut;
{
	...
	register int c, i;
	register char *pcMatch, *pcLine;

	if ((FILE *)0 == (fpFile = popen(pcCmd, "r"))) {
		fprintf(stderr, "%s: popen: %s: %s\n", progname, pcCmd, strerror(errno));
		return;
	}
	if ((PPM_BUF *)0 == util_ppm_init(& PMLine, sizeof(char), MAXPATHLEN+512)) {
		fprintf(stderr, "%s: %s: ppm init failed\n", progname, pcCmd);
		return;
	}
	...
	while (EOF != (c = getc(fpFile))) {
		pcLine = util_ppm_size(& PMLine, i+2);
		if ('\n' != c) {
			pcLine[i++] = c;
			continue;
		}
		...
		pcLine[i] = '\000';
		if ((char *)0 != (pcMatch = KindaLike(pcLine, " # include "))) {
			RpcRemember(pcMatch);
		} else if ((char *)0 != (pcMatch = KindaLike(pcLine, acHackMe)) && '\000' == *pcMatch) {
			RpcTimeout(fpOut);
		} else {
			fprintf(fpOut, "%s\n", pcLine);
		}
...
char *pcAsk;
{
	...
	(void)sprintf(acDupToken, "%lx/%lx", (unsigned long)stSeen.st_dev, (unsigned long)stSeen.st_ino);
	...
	iFd = mkstemp(acTFilex);
	if ((FILE *)0 == (fpDest = fdopen(iFd, "wb"))) {
	...
	strcpy(acBuf, acTFilex);
	strcat(acTFilex, ".x");
	if (0 != rename(acBuf, acTFilex)) {
...
char *pcBaseName, *pcManPage;
{
	...
	if ((FILE *)0 != fpCBody) {
		...
		if (nil != pchProgName && '\000' != pchProgName[0]) {
			fprintf(fpCBody, "%s", apcDef[iFound]);
			iFound = 1;
