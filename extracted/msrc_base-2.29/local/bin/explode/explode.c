/* $Id: explode.c,v 6.3 2008/03/14 19:38:07 ksb Exp $
...
#if USE_MKSTEMP
	if (-1 == (iFd = mkstemp(acTemp))) {
		fprintf(stderr, "%s: mkstemp: %s\n", progname, strerror(errno));
		exit(EX_TEMPFAIL);
	}
	pcTemp = acTemp;
#else
	/* Try to make mktemp as safe as we can				(ksb)
	 */
	if ((char *)0 == (pcTemp = mktemp(acTemp))) {
		fprintf(stderr, "%s: mktemp: %s\n", progname, strerror(errno));
		exit(EX_TEMPFAIL);
	}
	if (-1 == (iFd = open(pcTemp, O_RDWR, 0600))) {
		fprintf(stderr, "%s: open: %s: %s\n", progname, pcTemp, strerror(errno));
		(void)unlink(pcTemp);
		exit(EX_TEMPFAIL);
	}
	if (-1 == fchmod(iFd, 0600)) {
