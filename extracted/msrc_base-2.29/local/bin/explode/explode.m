...
%i
static char rcsid[] =
	"$Id: explode.m,v 6.6 2008/12/05 16:42:39 ksb Exp $";
%%
%h
...
char *pcFile;
{
	...
	register char *pc, *pcHas;
	register int i = MAXLANG;

	pcHas = strrchr(pcFile, '.');
	if (((char *)0 != (pc = pcExt) && !fFakeExt) || (char *)0 != (pc = pcHas)) {
		for (i = 0; i < MAXLANG; ++i) {
	...
	if (MAXLANG == i) {
		i = 0;			/* default to C		*/
		LITable[0].pcext = (char *)0 != pcHas ? pcHas : ".c";
	}
