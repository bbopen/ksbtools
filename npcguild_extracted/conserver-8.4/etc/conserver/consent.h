/*
 * $Id: consent.h,v 8.1 1998/06/07 16:10:38 ksb Exp $
 *
...
typedef struct consent {	/* console information			*/
	...
	int fuseflow;		/* use ^S/^Q rather than TANDEM		*/
#if DO_ANNEX
	/* used if we can chat with an Annex */
	int fannexed;		/* is on an Annex terminal server	*/
	int haxport;		/* the port this live on		*/
	char *pcannex;		/* the annex device name		*/
#endif
#if DO_VIRTUAL
...
#endif
#if DO_POWER
	char acpower[8];	/* e.g.: B1 is unit B, relay 1		*/
	char acctlr[MAXSERVLEN+MAXHOSTNAMELEN+2]; /* line@conserver	*/
#endif
	/* only used in child */
...
extern int XlateSpeed(/* char *, BAUD **, PARITY ** */);
extern void ConsInit();
extern void ConsDown();
extern void BaudRate(/* FILE * */);
