/* $Id: preend.c,v 3.10 1997/03/18 19:01:40 ksb Exp $
...
extern char *malloc(), *strchr(), *strcpy();
...
daemonize()
{
	...
	if (-1 == (res = fork())) {
		(void) fprintf(stderr, "%s: couldn't start\n", progname);
...
char **ppc;
{
	...
	if (NULL == (pwent = getpwnam(TOMB_OWNER))) {
		fprintf(stderr, "%s: getpwname: %s: %s\n", progname, TOMB_OWNER, strerror(errno));
		exit(2);
	}
	...
	pTIList = FsList(gidCharon, (dev_t)NOT_A_DEV);
}
...
Version()
{
	...
	printf("%s: %s\n", progname, "$Id: preend.c,v 3.10 1997/03/18 19:01:40 ksb Exp $");
	printf("%s: tombs must be owned by %s (uid %d, gid %d)\n", progname, TOMB_OWNER, uidCharon, gidCharon);
	printf("%s: tombs on this machine:\n", progname);
...
char **argv;
{
	...
	if (argc != 0) {
		...
		for (i = 0; i < argc; ++i) {
			...
			if (uidCharon != stTomb.st_uid) {
				fprintf(stderr,  "%s: %s: must be owned by %s (tomb %d != uid %d)\n", progname, pcTodo, TOMB_OWNER, stTomb.st_uid, uidCharon);
				continue;
			}
			if (gidCharon != stTomb.st_gid) {
				fprintf(stderr,  "%s: %s: must be grouped to %s (tomb %d != gid %d)\n", progname, pcTodo, TOMB_OWNER, stTomb.st_gid, gidCharon);
				continue;
			}
...
#define major(Mm)	((Mm >> 8) & 0xff)
	...
	if (fSingle) {
		...
		for (pTI = pTIDo; 0 != pTI; pTI = pTI->pTINext) {
			 ...
			 * the tomb to optimize namei not to go through so
			 * much code in full path name. So:
