/* $Id: vinst.mc,v 1.7 1999/06/01 17:27:17 ksb Exp $
...
struct stat *pstTarget;
{
	...
	if (pstTarget->st_size != stCopy.st_size) {
		fprintf(stderr, "%s: copy is not the same size (%d != %d)\n", progname, pstTarget->st_size, stCopy.st_size);
		return 1;
	}
	return 0;
}
...
int iStyle;
{
	...
	(void)sprintf(acCmd, "%s %s\"%s\" \"%s\" | ${PAGER-${more-more}}", acPathDiff, 'd' == iStyle ? "" : "-C3 ", pcF1, pcF2);
	system(acCmd);
}
...
char **ppcInstOpts;
{
	...
	auto int fdCopy;
	auto char *pcHards;
	auto char **ppcNewArgv, *pcInstall;
	extern char **environ;
	...
	ppcNewArgv = (char **)calloc(argc+10, sizeof(char *));
	if ((char **)0 == ppcNewArgv) {
		fprintf(stderr, "%s: calloc: %d*%d: %s\n", progname, argc+4, sizeof(char *), strerror(errno));
		exit(1);
	}
	...
	if (-1 != stat(acOld, & stOld) && S_IFDIR == (stOld.st_mode & S_IFMT)) {
		/* all is well, we think
		 */
	} else if (ENOENT != errno) {
		fprintf(stderr, "%s: stat: %s: %s\n", progname, acOld, strerror(errno));
		exit(2);
	} else {
		auto pid_t wPid, iStatus;
		...
		switch (wPid = fork()) {
		case -1:
			/* ignore it -- install might build it */
			break;
		case 0:
	...
	*pcEnd++ = '/';
	 ...
	 * how smart should be be here.  I pick to show off, a little.
	 */
	if (-1 != (fdCopy = open(acTemp, O_CREAT|O_WRONLY, -1 == fTarget ? 0775 : (0600|(stTarget.st_mode & 0777))))) {
		/* OK */
	} else {
	 ...
	 *	e edit it again
	 *	d show me the diffs ([c] context changes)
	 *	h help [?] help
	 *	i install [y] yes
...
%		iEdit = %K<util_editor>1v(acTemp)%;
		...
		/* fall through */
	case 'a':
		fprintf(stderr, "%s: What next? [?, d/c, e, i, q, r] ", progname);
		i = '\000';
		do {
			iEdit = getchar();
			if (!isspace(iEdit) && '\000' == i) {
	...
	default:
		...
		printf("d  show differences between copy and target [c context]\n");
		printf("e  edit the file again\n");
		printf("i  update %s\n", pcTarget);
		printf("r  reload %s, edit a frash copy\n", acTemp);
		printf("?  this help message\n");
		i = 'a';
		continue;
	} break; }
