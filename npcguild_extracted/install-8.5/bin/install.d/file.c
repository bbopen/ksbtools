 ...
 */
#if !defined(lint)
static char *rcsid = "$Id: file.c,v 8.6 1997/10/15 01:28:08 ksb Exp $";
#endif	/* !lint */
 ...
 *  "ksb", "ZZZ", "foo:bar:ksb:wham"  -> "foo:bar:ZZZ:wham"
...
char *pcLink;			/* in/out list to replace		*/
{
	...
	*pcLink = '\000';
	...
	pcMem = malloc(((char *)0 == pcInsert ? 0 : (1 + strlen(pcInsert))) + strlen(pcOrig) + 1 + strlen(pcColon+1)+2);
	if ((char *)0 != pcMem) {
		if ('\000' != *pcOrig) {
...
struct group *grp;		/* group for file			*/
{
	...
	auto char *pcFile;		/* file name to link to		*/
	...
	for (pcLink = pcLinks; '\000' != pcLink[0]; *pcColon = ':', pcLink = pcColon+1) {
		...
		*pcColon = '\000';
		...
		if (SOFT != eType || (char *)0 != strchr(pcLink, '/')) {
			pcFile = pcFullFile;
		} else if (0 == strcmp(pcBase, pcLink)) {
		...
		if (-1 != STAT_CALL(pcLink, &statb_link)) {
			...
			case S_IFLNK:	/* symbolic link */
				...
				if (-1 == stat(acLink, & statb_p2)) {
					...
					bBackup = TRUE;
				} else if (0 != strcmp(pcFile, acLink)) {
					if (FALSE == fQuiet)
...
fix_mode:
			if (FALSE != bHaveRoot) {
				ChOwnGrp(pcLink, pwd, grp);
			} else {
				ChGroup(pcLink, grp);
			}
#else
			(void)fprintf(stderr, "%s: symbolic links not supported\n", progname);
			fRet = FAIL;
#endif
		}
		if (FALSE != fVerbose) {
...
struct passwd *pwd;
{
	...
	case 0:		/* child, go do linking			*/
		...
		if (pcLash != acSlash && pcLash != acDot) {
			*pcLash = '/';
		}
...
char *pcSLink;		/* symlinks to make				*/
{
	...
	if (-1 != STAT_CALL(acDestPath, &statb_dest)) {
		...
		*pcSlash = '\000';
		if (-1 == stat(acBackPath, &statb_dest)) {
			...
			*pcHelp = '\000';
			if (-1 == stat(acDestPath, &statb_dest)) {
		...
		*pcSlash = '/';
		...
		if (FALSE == bLocalCopy) {
			if (FALSE != fTrace) {
				...
				/* not quiet because we `failed' here */
	 ...
	 */
	if (FALSE != bHaveRoot) {
		ChOwnGrp(pcFilePath, pwd, grp);
	} else {
		ChGroup(pcFilePath, grp);
	}
	...
	if (FAIL == Rename(pcFilePath, acDestPath, (char *)0)) {
		 ...
		 */
		(void)fprintf(stderr, "%s: no currently installed file, aborting\n", progname);
		exit(99);
	}
	...
	if (-1 != STAT_CALL(acBackPath, & statb_dest) && 1 != statb_dest.st_nlink) {
		if ((char *)0 != pcHLink) {
			(void)printf("%s: -H option may not have listed all the links to %s, still %d left\n", progname, acDestPath, statb_dest.st_nlink - 1);
		} else {
			(void)printf("%s: %s may still be installed someplace, link count is too big (%d)\n", progname, acBackPath, statb_dest.st_nlink);
		}
		*pcSlash = '\000';
	...
	if (FALSE == bWasLink && FALSE != bDestExists && FALSE != bBackup && 0 != (statb_dest.st_mode & ~SAFEMASK)) {
		ChMode(acBackPath, (int)statb_dest.st_mode & SAFEMASK);
	}
