 ...
 */
#if !defined(lint)
static char *rcsid = "$Id: syscalls.c,v 8.2 1997/02/15 21:05:15 ksb Exp $";
#endif	/* !lint */
 ...
 *
 * Globals:
 *	Group	- character string specifying the group owner
 */
void
...
{
	if (RunCmd(acChgrp, pgroup->gr_name, pcFile) != 0) {
...
char *Dest;		/* the target (destination) file		*/
{
	...
	/* Do the copy here
	 */
	while (0 != (n = read(fdSrc, acBlock, BLKSIZ))) {
		if (-1 == n) {
...
/*
 * call mktemp is fTrace is not set, else replace XXXX... with $$	(ksb)
 */
char *
Mytemp(pcFile)
char *pcFile;
