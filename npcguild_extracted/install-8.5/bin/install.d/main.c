...
#if !defined(lint)
static char *rcsid = "$Id: main.c,v 8.3 1997/10/15 01:28:08 ksb Beta $";
#endif	/* !lint */
...
FILE *fp;
{
	...
	(void)fprintf(fp, "%s: version: %s\n", progname, "$Id: main.c,v 8.3 1997/10/15 01:28:08 ksb Beta $");
#if defined(CONFIG)
...
char **argv;
{
	...
	/* Check for environtment options
	 */
	...
	while (EOF != (iOption = n_getopt(argc, argv, Opts))) {
		...
		case 'S':	/* soft links	*/
#if HAVE_SLINKS
			SoftLinks = OptAccum(SoftLinks, optarg, ":");
			break;
		...
		case 'H':	/* hard links	*/
			HardLinks = OptAccum(HardLinks, optarg, ":");
			break;
		case 'd':	/* directory	*/
			BuildDir = TRUE;
			break;
	...
	if (FALSE != fExplain) {
		...
		if (0 == strcmp(OLDDIR, StrTail(pcDir))) {
			 ...
			 */
			if (FAIL == DirInstall(pcDir, FALSE == bHaveRoot ? (char *)0 : ODIROWNER, FALSE == bHaveRoot ? (char *)0 : ODIRGROUP, ODIRMODE, (char *)0, (char *)0, (char *)0, SoftLinks, fDelete)) {
				exit(1);
			}
		...
		if (FAIL == Install("-", argv[optind], HardLinks, SoftLinks)) {
			++iFailed;
		}
	} else {
		if (iArgs < 2) {
		...
		/* install the files
		 */
		for (--argc; optind < argc; optind++) {
			if (FAIL == Install(argv[optind], Dest, HardLinks, SoftLinks)) {
