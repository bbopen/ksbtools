...
#if !defined(lint)
static char rcsid[] =
	"$Id: old.c,v 8.24 1999/02/06 23:46:21 ksb Exp $";
#endif	/* !lint */
 ...
 * the 1200 line of code we run for hard link repaiers, this is sane) by
 ...
 * If we find subdirs, char/block specials, sockets or the like --
...
int fDown;	/* 'n' (no), '?' (ask user), 'y' (yes) */
{
	...
	auto struct stat stIn, stBackFile;
	register DIR *pDI;
	register struct DIRENT *pDE;
	register char *pcTail;
	register int iNot, iPlain, iSpecial, iDir, iFifo;
#if HAVE_SYMLINKS
	register int iSlink;
#endif
	...
	if (2 < pstDir->st_nlink && '?' == fDown) {
		...
		fprintf(fpOut, "%s: backup subdirectory %s/%s has %d descendent director%s\n", progname, pcIn, pcDir, pstDir->st_nlink-2, 3 == pstDir->st_nlink ? "y" : "ies");
	...
	iNot = iPlain = iSpecial = iDir = iFifo = 0;
#if HAVE_SYMLINKS
	iSlink = 0;
#endif
	*pcTail++ = '/';
	...
	*--pcTail = '\000';
	...
	if (pstDir->st_nlink != (iDir + 2)) {
		fprintf(stderr, "%s: %s/%s failed internal link count sanity check (%d != %d)\n", progname, pcIn, pcDir, pstDir->st_nlink, (iDir + 2));
		return fWhack;
	}
	/* if all any specials or dirs we just bail
	 */
	if (0 != iSpecial || 0 != iDir) {
...
struct DIRENT **ppDELeft, **ppDERight;
{
...
CHLIST *pCLCheck;		/* the checklist buffer DoCk uses	*/
{
	...
	for (i = 0; i < iCount; ++i, chdir(acPwd)) {
		 ...
		 */
		if ('/' == ppcDirs[i][0]) {
			(void)sprintf(acCurOld, "%s/%s", ppcDirs[i], acOLD);
		} else if ('.' == ppcDirs[i][0] && '\000' == ppcDirs[i][1]) {
			(void)sprintf(acCurOld, "%s/%s", acPwd, acOLD);
		} else {
			(void)sprintf(acCurOld, "%s/%s/%s", acPwd, ppcDirs[i], acOLD);
		}
		/* if the dir is a symlink we need to modes on the remote
		 * and to clone, not the modes on the link itself	(ksb)
		 */
		if (-1 == stat(ppcDirs[i], & stDir)) {
			(void)fprintf(fpOut, "%s: stat: %s: %s\n", progname, ppcDirs[i], strerror(errno));
			continue;
		}

		/* check the modes on the OLD directory itself
		 */
		if ((char *)0 == pcOldOwner) {
		...
		if (stOld.st_dev != stDir.st_dev) {
			(void)fprintf(fpOut, "%s: `%s' is on another device, device of parent directory differs (%d != %d)\n", progname, acCurOld, stOld.st_dev, stDir.st_dev);
			/* ask if they'd like us to umount it :-) */
			continue;
		}
		(void)DoCk(acCurOld, &stOld);

		/* goto the directory and seach for poor backups
		 */
		if (-1 == chdir(acCurOld)) {
			if (ENOTDIR != errno)
		...
		for (j = 0; iConsumed = 1, j < n; j += iConsumed) {
...
#else
			(void)sprintf(acMode, "%04o", PERM_RWX(stBackup.st_mode));
#endif
			apcChmod[2] = acMode;
