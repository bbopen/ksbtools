...
#if !defined(lint)
static char rcsid[] =
	"$Id: gen.c,v 8.13 1999/03/06 19:23:15 ksb Exp $";
#endif	/* !lint */
...
struct DIRENT *pDE;
{
	if (_SamePath(_pcGlTest, pDE->d_name, 1)) {
		return 1;
	}
	return 0;
}
 ...
 */
static int
DirOrder(pGRLeft, pGRRight)
GRAB *pGRLeft, *pGRRight;
{
 ...
 *	?---------	why mode 0?		[is none other]
 *
 * We need 4 fields from the stat struct in the param list:
 *	st_uid, st_gid, st_mode, st_nlink
 * If we even need more we'll have to fix the call in GenCk, which doesn't
...
int iCount;
{
	...
	if ('-' == pcMode[0] && 0 != (pstThis->st_mode & 02000) && (0 == (pstThis->st_mode & 0101) || 0 == pwdThis->pw_uid && 0 == (pstThis->st_mode & 01)) && 0 != (pstThis->st_mode & 0010)) {
		if (!fCom)
			(void)printf(acSCom);
	...
	if (0 != iCount) {
		if (!fCom)
			(void)printf(acSCom);
		(void)printf(" %d link%s of %d not shown", iCount, 1 == iCount ? "" : "s", pstThis->st_nlink);
		++fCom;
	}
	if ('d' == pcMode[0] && (
	...
	/* we might do something with fCom here: we could output
	 * an exclaimation mark (!) here if you have more than 4 comments?
	 */
	(void)fputc('\n', stdout);
}
 ...
 * (also output if fLinks and st_nlink > 1)
 */
static int
...
int iCount;
{
	...
	if (!fLongList && MESame.chtype == ch && MESame.iuid == pstThis->st_uid && MESame.igid == pstThis->st_gid && MESame.imode == PERM_BITS(pstThis->st_mode) && !(fLinks && 1 < pstThis->st_nlink)) {
		return 0;
	}
...
char **argv;
{
	 ...
	 * (in the multiple link case (very much like the loop at line 281).
	 */
	for (j = i = 0; i < iBox; fBasePush && (PopModes(& PMFile), 0), ++i) {
		...
		if (fDirs && S_IFDIR == (pGR[i].stthis.st_mode & S_IFMT)) {
			...
			if ((MAXFREQ *)0 == ppMFStats[i] || 1 == ppMFStats[i]->icount || fLongList) {
				if (fRecurse) {
					GenMatch("*", pcFile, DoDiffs);
				} else {
					GenMatch("*", pcFile, DoGen);
				}
				PopModes(& PMDir);
				continue;
			}
