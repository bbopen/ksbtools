/* $Id: links.c,v 8.12 1999/06/20 10:53:07 ksb Exp $
 *
...
struct stat *pstBack, *pstOrig;
{
	...
	pcOrig[0] = '\000';
	(void)sprintf(acLoop, "%s/%s", pcIn, pcBack);
	iLen = strlen(acLoop);
...
int iNeed;
{
	...
	i = scandir(pcTop, &ppDE, (void *)0, (void *)0);
	if (-1 == i) {
		return iNeed;
	}
...
register LINK *pLIList;
{
	 ...
	 */
	iCount = iFill = iUnique = 0;
	for (i = 0; i < iBacks; ++i) {
		pLI = & pLIList[iFill++];
		sprintf(pLI->acback, "./%s", ppDEBacks[i]->d_name);
		pLI->pcbtail = pLI->acback+1;
		pLI->pstold = pstBackup;
		pLI->ppDEold = & ppDEBacks[i];
		++iCount;
		(void)sprintf(pLI->acinst, "../%s", pLI->pcbtail+1);
	...
	if (iCount < pstBackup->st_nlink) {
		...
		for (i = 0; i < iInsts; ++i) {
			...
			pLI = & pLIList[iFill++];
			(void)sprintf(pLI->acinst, "../%s", pcLink);
			pLI->pcitail = pLI->acinst+2;
			pLI->ppDEup = & ppDEInsts[i];
			pLI->pstup = pstBackup;
			++iCount;
			(void)sprintf(pLI->acback, "./%s", pcLink);
	 ...
	 */
	if (iCount != pstBackup->st_nlink) {
		fprintf(fpOut, "%s: %s: internal track error %d != %d\n", progname, ppDEBacks[0]->d_name, iCount, pstBackup->st_nlink);
		iExitCode |= 32;
		return 0;
	}
	for (i = 0; i < iFill; ++i) {
