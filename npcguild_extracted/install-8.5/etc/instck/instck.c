...
#if !defined(lint)
static char rcsid[] =
	"$Id: instck.c,v 8.18 1999/02/06 23:46:21 ksb Exp $";
#endif	/* !lint */
...
/*
 * called if the glob match code didn't find any new files		(ksb)
 */
void
NoMatches(pcGlob)
char *pcGlob;
...
/*
 * sun's chmod(1) won't take litteral 0755 to change setgid bits	(ksb)
...
int *piStatus;
{
	...
	if (bHaveRoot) {
		...
		iTotal += 3 + strlen(pcGuilty);	/* "by " + guilty */
		iTotal |= 7;
		++iTotal;
		...
		if ((char *)0 == pcLogMem) {
			syslog(LOG_INFO, "too long %s by %s", ppcArgv[0], pcGuilty);
			return QE_RAN;
		}
...
char *pcIn, *pcFile;
{
	...
	if (-1 == stat(acOLD, & stOld)) {
		...
		fprintf(stderr, "%s: %s/%s should be a directory!\n", progname, pcIn, acOLD);
		return 0;
	}
	/* de install the file
	 */
...
/*
 * we are passed a filename that exists and matches the recorded
...
struct stat *pstThis;
{
	...
	if ('~' == CLCheck.acmode[0]) {
		...
		/* if the user didn't remove it, check it LLL check exit code
		 */
...
/* check for files that were not checked:				(ksb)
 *	wide open directory
 ...
 * the default group is bad? (ksb LLL)
 */
int
...
struct stat *pstThis;
{
	...
	if ('d' == acMode[0] && 0777 == (pstThis->st_mode & 00777) && 0 == (pstThis->st_mode & 01000)) {
		...
		fprintf(fpOut, ")\n");
	} else if ('l' != acMode[0] && (0 != (pstThis->st_mode & ('d' == acMode[0] ? 01022 : 07022)) || 0 == (pstThis->st_mode & 07777))) {
		ModetoStr(acMode+1, (int)PERM_BITS(pstThis->st_mode), 0000);
		fprintf(fpOut, "%s: `%s\' unchecked %s, mode %s (%04o", progname, pcFile, pcType, acMode, PERM_BITS(pstThis->st_mode));
		...
		static char *apcUnMap[4] = {
			"ouch", "owner", "group", "owner and group"
		};
		...
		if ((0700 & pstThis->st_mode) != ((0070 & pstThis->st_mode) << 3) || (0700 & pstThis->st_mode) != ((0007 & pstThis->st_mode) << 6)) {
			fprintf(fpOut, " (mode %04o)", PERM_BITS(pstThis->st_mode));
		}
		fprintf(fpOut, "\n");
		...
		fprintf(fpOut, "%s: `%s\' %s is world accessible (%04o)\n", progname, pcFile, pcType, PERM_BITS(pstThis->st_mode));
	} else if (fLongList) {
...
/*
 * sep the dirs and the files, dirCk the dirs, fileck file files	(ksb)
 */
void
